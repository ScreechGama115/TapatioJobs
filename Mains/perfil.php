...
if(isset($_GET['id'])) {
	...
	$coment = mysql_query("SELECT * FROM mensajes WHERE id_count = '".$_GET['id']."'");
	$com=mysql_fetch_array($coment);

	if ($com['type'] == 1) {
		$query = mysql_query("SELECT * FROM business WHERE id_count = '".$_GET['id']."' AND type = 1");
	}elseif ($com['type'] == 2) {
		$query = mysql_query("SELECT * FROM users WHERE id_count = '".$_GET['id']."' AND type = 2");
	}
...
<center> <h1 style="text-decoration:underline">Datos de Curriculum</h1> </center>
	...
	<p>
	<?php if($_SESSION['id_count'] == $_GET['id'] && $com['type'] == $_SESSION['type']) {?>
